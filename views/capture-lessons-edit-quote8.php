...
if ($_POST['r'] == 'capture-lessons-edit-quote8'   && !isset($_POST['crud'])) {

	$tbc = $tbc_controller->get8($_POST['id_title_bible_quote8']);
	...
	if (empty($tbc)) {
		...
		printf($template, $_POST['title_bible_quote8']);
	} else {
		...
		$template_tbc = '
			...
			<form method="POST" class="">
				<div class="">
					<input type="text" placeholder="id_title_bible_quote8" value="%s" disabled required>
					<input type="hidden" name="id_title_bible_quote8" value="%s">
				</div>
				<div class="">
					<input type="text" name="title_bible_quote8" placeholder="Objetivos" value="%s" required>
				</div>
				<div class="">
				...
				<div class="p_25">
					...
					<input type="hidden" name="r" value="capture-lessons-edit-quote8">
		...
		printf(
			$template_tbc,
			$tbc[0]['id_title_bible_quote8'],
			$tbc[0]['id_title_bible_quote8'],
			$tbc[0]['title_bible_quote8'],
			$name_lesson_select,
			$number_question_select
		);
	}
} else if ($_POST['r'] == 'capture-lessons-edit-quote8'   && $_POST['crud'] == 'set') {
	...
	$tbc = $tbc_controller->set8($save_tbc);
